...
def column_dimensions_calculations(feed_flow, xf, xd):
    ...
    qr=(bo1*(B/M1)*latent_heat_vapour) # Units - kJ/hr, converting our flow rate to molar flow rate (B/Molar mass)                          
    ...
    qc=(reflux*(D/M2)*latent_heat_liquid) # Units- kJ/hr, converting our flow rate to molar flow rate (D/M)
    ...
    max_vapor_velocity=k*(((density_liquid-density_vapour)/density_vapour)**0.5)

    diameter=np.sqrt((4*vapor_mass_flow_rate)/(np.pi*density_vapour*max_vapor_velocity))
...
def column_dimensions(feed_flow,xf,xd):
    ...
    return(print(f"Length of column = {a[0]:1.3f} m\nDiameter of column = {a[1]:1.3f} m"))
